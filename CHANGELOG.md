...
- **Requiring `quic.cr` unconditionally broke `crystal spec`/`crystal build` on CI**, found by the release workflow itself right after tagging v0.2.4: `quic.cr`'s TLS bindings need OpenSSL's native QUIC API (`SSL_set_quic_tls_cbs` and friends, added in OpenSSL 3.5), which CI's `crystallang/crystal:1.20` image doesn't ship — the link step failed with `undefined reference to 'SSL_set_quic_tls_cbs'` for the whole shard, not just HTTP/3-specific code, because `src/morganite.cr` required it unconditionally regardless of the runtime `jqcp_http3_enabled` flag. Gated the require behind a compile-time flag (`-Dmorganite_http3`) in both `morganite.cr` and `launcher.cr`; the default build (this shard's own `make build`, and CI) no longer touches `quic.cr` at all.
